...
Установка SREBot ориентирована на Kubernetes-инфраструктуру. Бот для Telegram, Slack, Discord или
Time разворачивается с помощью **Helm**, что обеспечивает изолированную работу AI-агента внутри
вашей приватной сети.
...
2. **Учетные данные чат-платформы:**
   - Выберите одну платформу и подготовьте ее токены и ID канала.
   - Для Telegram следуйте [инструкции Telegram](/guide/telegram-setup).
   - Для Time следуйте [инструкции Time Messenger](/guide/time-setup).

Один процесс SREBot может быть подключен только к одной чат-платформе.

Если доступ к чат-сервису идёт через HTTP-прокси, настройте `HTTPS_PROXY` в окружении
контейнера бота. Примеры и ограничения приведены в разделе
[HTTP-прокси](./configuration#http-proxy).
...
secrets:
  ...
  saas_agent_token: "ВАШ_AGENT_TOKEN_ИЗ_ДАШБОРДА"
...
Для Time замените секцию `secrets`:

```yaml
secrets:
  time_base_url: "https://time.example.com"
  time_token: "ТОКЕН_TIME-БОТА"
  time_channel_id: "ID_TIME-КАНАЛА"
  saas_agent_token: "ВАШ_AGENT_TOKEN_ИЗ_ДАШБОРДА"
```
...
1. Он авторизуется в выбранном мессенджере и подключится к настроенному каналу.
...
Добавьте учетную запись бота в целевой инцидент-чат. Бот начнет работу при первом сработавшем
инциденте.
:::
